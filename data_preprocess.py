            ...
            if utter_name[-5:] == '.flac':
                ...
                for interval in intervals:
                    ...
                    if (interval[1]-interval[0]) > utter_min_len:           # If partial utterance is sufficient long,
                        ...
                        file_spec.append(S[:, :hp.data.tisv_frame])
                        #file_spec.append(S[:, -hp.data.tisv_frame:])
                        file_spec = np.array(file_spec)
                        print("Saving", j)
                        if i<train_speaker_num:      # save spectrogram as numpy file
                            np.save(os.path.join(hp.data.train_path, "speaker{}_{}.npy".format(i, j)), file_spec)
                        else:
                            np.save(os.path.join(hp.data.test_path, "speaker{}_{}.npy".format(i-train_speaker_num, j)), file_spec)
                    else:
...
def get_spectrogram_tisv(utter_path):
    """ Full preprocess of text independent utterance. The log-mel-spectrogram is saved as numpy file.
        Each partial utterance is splitted by voice detection using DB
        and the first and the last 180 frames from each partial utterance are saved.
        Need : utterance data set (VTCK)
