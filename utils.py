...
def mfcc_for_accent(utter):
    ...
    if(diff>=0):
      ext = utter[0:diff]
      out=np.append(utter,ext)
    else:
      out = utter[0:59448]
      #limit = 59448 + diff
      #ext = utter[59448:limit]
      #out = np.append(out,ext)

    utter = out
    assert utter.shape[0]==59448, utter.shape[0]
    mfc = librosa.feature.melspectrogram(utter,n_fft= 2048,hop_length=512, n_mels=128)
      #l.append(1)
      #indian.append(l)
      #print(ctr)
    return utter, mfc
